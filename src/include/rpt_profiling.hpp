...
public:
	...
	shared_ptr<CreateBFStats> RegisterCreateBF(idx_t build_table_idx, idx_t probe_table_idx) {
	...
	shared_ptr<UseBFStats> RegisterUseBF(idx_t build_table_idx, idx_t probe_table_idx) {
		lock_guard<mutex> lock(stats_lock);
	...
	void PrintSummary() {
		...
		if (!create_bf_stats.empty()) {
			Printer::Print("\nCREATE_BF:");
			for (auto &s : create_bf_stats) {
				Printer::PrintF("  [build=table_%llu -> probe=table_%llu] %llu rows, sink=%lldus, finalize=%lldus, source=%lldus",
		...
		if (!use_bf_stats.empty()) {
			Printer::Print("\nUSE_BF:");
			for (auto &s : use_bf_stats) {
				idx_t ri = s->rows_in.load();
				idx_t ro = s->rows_out.load();
				double sel = ri > 0 ? 100.0 * (double)ro / ri : 0.0;
				Printer::PrintF("  [build=table_%llu, probe=table_%llu] in=%llu, out=%llu, sel=%.1f%%, probe=%lldus",
				    (unsigned long long)s->build_table_idx,
				    (unsigned long long)s->probe_table_idx,
